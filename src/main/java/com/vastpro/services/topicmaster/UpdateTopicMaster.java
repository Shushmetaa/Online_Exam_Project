...
public class UpdateTopicMaster {
    ...
    public static Map<String, Object> updateTopic(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        try {
            ...
            // ✅ Fresh mutable map — never cast and mutate the OFBiz context
            Map<String, Object> serviceCtx = new HashMap<>();
            serviceCtx.put("examId",              examId);
            serviceCtx.put("topicId",             topicId);
            serviceCtx.put("topicName",           topicName);
            serviceCtx.put("percentage",          percentage);
            serviceCtx.put("startingQid",         startingQid);
            serviceCtx.put("endingQid",           endingQid);
            serviceCtx.put("topicPassPercentage", topicPassPercentage);
            serviceCtx.put("questionsPerExam",    String.valueOf(questionsPerExam));
            serviceCtx.put("userLogin",           userLogin);

            Map<String, Object> result = dispatcher.runSync("updateTopicMasterAuto", serviceCtx);
